...
class SkillExecuteRequest(BaseModel):
    skill_name: str = Field(..., min_length=1, description="Skill 名称，如 content_strategist / data_analyst")
    method: str = Field(..., min_length=1, description="要调用的具体方法名")
...
class SkillInfo(BaseModel):
    ...
    name: str = Field(..., description="Skill 中文名称")


class SkillListResponse(BaseModel):
    single_account_agents: list[SkillInfo] = Field(..., description="单账号代理")
    matrix_agents: list[SkillInfo] = Field(..., description="矩阵代理")
    utility_agents: list[SkillInfo] = Field(..., description="工具代理")


@router.post("/execute", response_model=SkillExecuteResponse, summary="执行 Skill 方法")
...
@router.get("/list", response_model=SkillListResponse, summary="列出所有可用 Skills")
async def list_skills():
    ...
    return SkillListResponse(
        single_account_agents=[
            SkillInfo(id="content_strategist", name="内容策略师"),
            SkillInfo(id="creative_studio", name="创意工厂"),
            SkillInfo(id="data_analyst", name="数据分析师"),
            SkillInfo(id="growth_hacker", name="投放优化师"),
            SkillInfo(id="community_manager", name="用户运营官"),
            SkillInfo(id="compliance_officer", name="合规审查员"),
        ],
        matrix_agents=[
            SkillInfo(id="matrix_commander", name="矩阵指挥官"),
            SkillInfo(id="bulk_creative", name="批量创意工厂"),
            SkillInfo(id="account_keeper", name="账号维护工"),
            SkillInfo(id="traffic_broker", name="流量互导员"),
            SkillInfo(id="knowledge_miner", name="知识提取器"),
            SkillInfo(id="sop_evolver", name="SOP进化师"),
        ],
        utility_agents=[
            SkillInfo(id="rpa_executor", name="RPA执行器"),
        ]
    )
